...
class RewardModel:
    ...
    def _forward(self, x: np.ndarray) -> np.ndarray:
        """前向传播"""
        # 第一层
        z1 = np.dot(self.model["W1"], x) + self.model["b1"]
        a1 = np.tanh(z1)
    ...
    def _update_batch(
        ...
        for state, reward in batch_data:
            # 前向传播
            reward_pred = self._forward(state)
            ...
            # 反向传播
            grad = self._compute_gradient(state, reward, reward_pred)
    ...
    def _compute_gradient(
        ...
        """计算梯度"""
        # 前向传播（缓存中间值）
        z1 = np.dot(self.model["W1"], state) + self.model["b1"]
        a1 = np.tanh(z1)
        ...
        # 隐藏层
        grad_hidden = np.dot(self.model["W2"].T, delta_loss) * (1 - a1 ** 2)
